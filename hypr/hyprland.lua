...
hl.config({
  general = {
    ...
    gaps_out = 10,
    border_size = 2,
    ["col.active_border"] = { colors = { "rgba(d35400ff)", "rgba(d35400ff)" }, angle = 45 },
...
local animations = {
  ...
  { "workspaces", 1.94, "easeInOutCubic", "slide" },
  { "workspacesIn", 1.21, "easeInOutCubic", "slide" },
  { "workspacesOut", 1.94, "easeInOutCubic", "slide" },
  { "zoomFactor", 4, "quick" },
}
...
bind("SUPER + SHIFT + W", hl.dsp.window.float({ action = "toggle" }))
...
bind("SUPER + ALT + SHIFT + W", exec('xfreerdp3 /u:"tere" /p:"isg2025" /v:127.0.0.1:3389 -grab-keyboard /sound /microphone /cert:ignore /title:"Windows VM" /dynamic-resolution /gfx:AVC444 /floatbar:sticky:off,default:visible,show:fullscreen'))
bind("SUPER + slash", exec("uwsm app -- proton-pass"))
bind("F8", exec("hyprshot -m region --clipboard-only"))

for _, direction in ipairs({ "left", "right", "up", "down" }) do
  bind("ALT + TAB + " .. direction, hl.dsp.focus({ direction = direction }))
  bind("SUPER + SHIFT + " .. direction, hl.dsp.window.move({ direction = direction }))
end

local shortcuts = {
  { "C", "CTRL", "Insert" }, { "V", "SHIFT", "Insert" }, { "Z", "CTRL", "Z" },
  { "SHIFT + Z", "CTRL SHIFT", "Z" }, { "A", "CTRL", "A" },
  { "LEFT", "", "HOME" }, { "RIGHT", "", "END" },
  { "SHIFT + LEFT", "SHIFT", "HOME" }, { "SHIFT + RIGHT", "SHIFT", "END" },
}
for _, shortcut in ipairs(shortcuts) do
  bind("SUPER + " .. shortcut[1], hl.dsp.send_shortcut({ mods = shortcut[2], key = shortcut[3] }))
end
bind("CTRL + UP", hl.dsp.send_shortcut({ mods = "", key = "Prior" }))
bind("CTRL + DOWN", hl.dsp.send_shortcut({ mods = "", key = "Next" }))
bind("CTRL + SHIFT + UP", hl.dsp.send_shortcut({ mods = "SHIFT", key = "Prior" }))
bind("CTRL + SHIFT + DOWN", hl.dsp.send_shortcut({ mods = "SHIFT", key = "Next" }))
...
bind("SUPER + ALT + PERIOD", hl.dsp.focus({ workspace = "+1" }))
bind("SUPER + ALT + COMMA", hl.dsp.focus({ workspace = "-1" }))
bind("SUPER + TAB", hl.dsp.workspace.toggle_special("magic"))
bind("SUPER + SHIFT + TAB", hl.dsp.window.move({ workspace = "special:magic" }))
...
bind("XF86AudioRaiseVolume", exec(noctalia_ipc .. " volume increase"), { repeating = true, locked = true })
bind("XF86AudioLowerVolume", exec(noctalia_ipc .. " volume decrease"), { repeating = true, locked = true })
bind("XF86AudioMute", exec(noctalia_ipc .. " volume muteOutput"), { locked = true })
...
bind("Caps_Lock", exec("swayosd-client --caps-lock"), { release = true })
